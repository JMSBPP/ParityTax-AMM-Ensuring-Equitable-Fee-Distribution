...
contract UnlockedV4Quoter is IV4Quoter, BaseV4Quoter{
        ...
        returns (uint256 amountOut, uint256 gasEstimate)
    {
        ...
        try this._quoteExactInputSingle(params) {}
    ...
    {
        uint256 gasBefore = gasleft();
        try this._quoteExactInput(params) {}
        catch (bytes memory reason) {
            gasEstimate = gasBefore - gasleft();
            // Extract the quote from QuoteSwap error, or throw if the quote failed
            amountOut = reason.parseQuoteAmount();
    ...
    {
        uint256 gasBefore = gasleft();
        try this._quoteExactOutputSingle(params) {}
        catch (bytes memory reason) {
            gasEstimate = gasBefore - gasleft();
            // Extract the quote from QuoteSwap error, or throw if the quote failed
    ...
    {
        uint256 gasBefore = gasleft();
        try this._quoteExactOutput(params) {}
        catch (bytes memory reason) {
            gasEstimate = gasBefore - gasleft();
            // Extract the quote from QuoteSwap error, or throw if the quote failed
            amountIn = reason.parseQuoteAmount();
