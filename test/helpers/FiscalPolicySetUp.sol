// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;




import {IERC20} from "forge-std/interfaces/IERC20.sol";
import {IPoolManager} from "@uniswap/v4-core/src/interfaces/IPoolManager.sol";
import {IParityTaxHook} from "../../src/interfaces/IParityTaxHook.sol";
import {Constants} from "@uniswap/v4-core/test/utils/Constants.sol";
import {Currency} from "@uniswap/v4-core/src/types/Currency.sol";
import {BalanceDelta} from "@uniswap/v4-core/src/types/BalanceDelta.sol";
import {MockLPOracle} from "../mocks/MockLPOracle.sol";
import {UniformFiscalPolicy} from "../mocks/UniformFiscalPolicy.sol";
import {ParityTaxRouter} from "../../src/ParityTaxRouter.sol";
import {IV4Quoter} from "@uniswap/v4-periphery/src/interfaces/IV4Quoter.sol";
import {V4Quoter} from "@uniswap/v4-periphery/src/lens/V4Quoter.sol";
import {Deployers} from "@uniswap/v4-core/test/utils/Deployers.sol";
import {IPositionManager} from "@uniswap/v4-periphery/src/interfaces/IPositionManager.sol";
contract FiscalPolicySetUp is Deployers {

    MockLPOracle lpOracle;
    UniformFiscalPolicy fiscalPolicy;
    V4Quoter v4Quoter;
    ParityTaxRouter parityTaxRouter;


    address callbackSender;

    function deployAndApproveFiscalPolicy(
        address _callback_sender,
        IPoolManager _manager,
        IPositionManager _lpm,
        IParityTaxHook _parityTaxHook
    ) internal{
        callbackSender = _callback_sender;

        v4Quoter = new V4Quoter(
            _manager
        );

        parityTaxRouter = new ParityTaxRouter(
            _manager,
            IV4Quoter(address(v4Quoter)),
            _parityTaxHook
        );

        lpOracle = new MockLPOracle();

        fiscalPolicy = new UniformFiscalPolicy(
            _callback_sender,
            lpOracle,
            _lpm,
            _parityTaxHook
        );

        IERC20(Currency.unwrap(currency0)).approve(
            address(parityTaxRouter),
            Constants.MAX_UINT256
        );
        IERC20(Currency.unwrap(currency1)).approve(
            address(parityTaxRouter),
            Constants.MAX_UINT256
        );

   }

}