...
library LiquidityCallbackDataLibrary {
     ...
     * @param liquidityCallbackData The LiquidityCallbackData containing the hookData to validate.
     */
    function hookDataDecodesToEncodedTimeCommitment(
        LiquidityCallbackData memory liquidityCallbackData
    ) internal pure {
        bytes memory hookData = liquidityCallbackData.hookData;
        if (
            hookData.length !=
            TimeCommitment({
                isJIT: true,
     ...
     * @param liquidityCallbackData The LiquidityCallbackData containing the encoded TimeCommitment.
    ...
    function getTimeCommitment(
        ...
        hookDataDecodesToEncodedTimeCommitment(liquidityCallbackData);
        timeCommitment = liquidityCallbackData
            .hookData
     ...
     * @param liquidityCallbackData The callback data containing the user's intent.
    ...
    function isLookingToAddLiquidity(
        ...
        _isLookingToAddLiquidity =
            liquidityCallbackData.liquidityParams.liquidityDelta > 0;
    }
    ...
    /// @param encodedLiquidityCallbackData The bytes to decode.
    /// @return liquidityCallbackData The decoded LiquidityCallbackData.
    ///
    ...
    function fromBytesToLiquidityCallbackData(
        bytes memory encodedLiquidityCallbackData
    )
        internal
        pure
        returns (LiquidityCallbackData memory liquidityCallbackData)
    {
        liquidityCallbackData = abi.decode(
