...
contract TestTimeCommitment is Test {
    ...
    function test__fuzz__PLP(uint48 _plpTimeCommitmentValue) external {
        ...
        uint48 plpTimeCommitmentValue = uint48(
            bound(
                _plpTimeCommitmentValue,
                vm.getBlockTimestamp(),
                type(uint48).max - 1
            )
        );
        TimeCommitment plpTimeCommitment = toTimeCommitment(
            plpTimeCommitmentValue
        );
    ...
    function test__fuzz__Uninitialized(bytes32 _randomBytes32) external {
        ...
        // uninitialized
        bytes32 randomBytes32 = bytes32(
            bound(uint256(_randomBytes32), 0, type(uint256).max)
        );
        TimeCommitment emptyTimeCommitment = testPositionKeysTimeCommitments[
            randomBytes32
        ];
        assertTrue(UNINITIALIZED(emptyTimeCommitment));
        console.logBytes(
            abi.encode(timeCommitmentLibrary.set(emptyTimeCommitment))
        );
    }

    function test__fuzz__timeStamp(uint48 _timeCommitmentValue) external {
        TimeCommitment randomTimeCommitment = toTimeCommitment(
            _timeCommitmentValue
        );
        assertEq(
            uint256(vm.getBlockTimestamp()),
            uint256(timeStamp(randomTimeCommitment))
        );
    }
    ...
    function test__fuzz__ltGt(
        uint48 randomBlockTimeStampX1,
        uint48 randomBlockTimeStampX2,
        uint48 timeCommitmentValueX1,
        uint48 timeCommitmentValueX2
    ) external {
        //NOTE: UNINITIALIZED TC is the lowest bound
        vm.warp(randomBlockTimeStampX1);
        uint48 timeStampX1 = uint48(vm.getBlockTimestamp());
        TimeCommitment timeCommitmentX1 = timeCommitmentLibrary.set(
            toTimeCommitment(timeCommitmentValueX1)
        );
        console.log("First Time Commitment:", timeStamp(timeCommitmentX1));
        {
            ...
            TimeCommitment timeCommitmentX2 = timeCommitmentLibrary.set(
                toTimeCommitment(timeCommitmentValueX2)
            );
    ...
    function test__fuzz__addTimeCommitments(
        bytes32 randomKey,
        uint48 blockTimeStampX1,
        uint48 blockTimeStampX2,
        uint48 __timeCommitmentValueX1,
        uint48 __timeCommitmentValueX2
    ) external {
        //NOTE let's set the case when the lp initiates the
        // position for the fisrt time. This is
        vm.assume(
            (blockTimeStampX1 >= DEFAULT_BLOCK_TIMESTAMP) &&
                (blockTimeStampX1 <= blockTimeStampX2)
        );
        uint48 _timeCommitmentValueX1 = uint48(
            bound(__timeCommitmentValueX1, 0, type(uint48).max)
        );
        ...
        TimeCommitment timeCommitmentX1 = toTimeCommitment(
            _timeCommitmentValueX1
        );
        {
            console.log(
                "Time Commmitment TimeStamp:",
                timeStamp(timeCommitmentX1)
            );
            console.log(
                "Time Commitment Value:",
                timeCommitmentValue(timeCommitmentX1)
            );
        }
        ...
        TimeCommitment t1Plust2 = timeCommitmentLibrary.Add(
            emptyTimeCommitment,
            timeCommitmentX1
        );
        if (UNINITIALIZED(timeCommitmentX1)) {
            ...
            assertEq(
                ...
                TimeCommitment.unwrap(emptyTimeCommitment)
            );
        } else {
            // The other case is initiating a non empty position
            assertEq(
                TimeCommitment.unwrap(t1Plust2),
                TimeCommitment.unwrap(timeCommitmentX1)
            );
        }
        testPositionKeysTimeCommitments[randomKey] = t1Plust2;
        vm.stopPrank();

        uint48 _timeCommitmentValueX2 = uint48(
            bound(__timeCommitmentValueX2, 1, type(uint48).max)
        );
    }
}
