...
import "../simple-subscription-batch.sol";

/**
 * @title TestSimpleSubscription
 * @dev Foundry script to test the simplified subscription batch calling
 */
contract TestSimpleSubscription is Script {
    
    function run() external {
        console.log("=== Testing Simple Subscription Batch ===");
        ...
        try SimpleSubscriptionBatch.batchExecuteSubscriptions(
            IParityTaxHook(mockParityTaxHook),
            chainId,
            subscriptionTarget
        ) returns (bool success, bytes[] memory returnDataArray) {
            console.log("Success:", success);
            console.log("Return data array length:", returnDataArray.length);
            
            for (uint256 i = 0; i < returnDataArray.length; i++) {
                console.log("Return data %d length:", i, returnDataArray[i].length);
                console.logBytes(returnDataArray[i]);
            }
